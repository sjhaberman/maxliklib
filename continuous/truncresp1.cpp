...
f1v contresp1(const char &, const vec & y, const vec & beta);
f1v berresp1(const char & , const ivec & y, const vec & beta);
f1v truncresp1(const char & transform, const resp & y, const vec & beta)
{
    ...
    if(y.iresp(0)>0)
    {
       ...
       remainder=berresp1(transform,i,gamma);
    ...
    {
        result=contresp1(transform,y.dresp,beta);
    }
    return result;	
}

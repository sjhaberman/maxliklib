...
//predictor matrix x.  Penalized likelihood is found
//for quadratic penalty.
//Weight w is used.
...
//The penalty matrix is the symmetric and nonnegative-definite matrix Q.
//The target vector is b.
#include<armadillo>
using namespace std;
...
f2v genresplikq(const int & order, const vector<dat> & data, const xsel & obssel,
    const mat & Q, const vec & b, const vec & beta)
{
    ...
    beta1.set_size(p);
    beta1=b-beta;
    results.value=-dot(beta1,Q*beta1);   
    if(order>0)
    {
        results.grad.set_size(p);
        results.grad=2.0*Q*beta1;
    }
    if(order>1)
    {

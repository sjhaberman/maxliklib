//Find maximum likelihood estimates for response model with quadratic penalty.
...
f2v genresplikq(const int & , const vector<dat> & , const xsel & ,
     const mat & , const vec & , const vec &);
maxf2v genrespmleq(const int & order, const params & mparams,
    const char & algorithm, const vector<dat> & data, const xsel & obssel,
    const mat & Q,  const vec & b, const vec & start)
{
    maxf2v results;
    int p;
    p=start.n_elem;
    ...
    auto f=[&data,&obssel,&Q,&b](const int & order,const vec & start)
        {return genresplikq(order,data,obssel,Q,b,start);};
